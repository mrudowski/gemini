...
const DialogueOptionsWidget: React.FC<IDialogueOptions> = ({options, onOptionSelect, actorId}) => {
  ...
  const classes = classNames('DialogueOptionsWidget', 'DialogueOptionsWidget--short');
  ...
  return (
    <div className={classes}>
      {options
        .filter(option => option.when !== false)
        .map(option => {
          let isAsked = false;
          if (option.id !== 'end') {
